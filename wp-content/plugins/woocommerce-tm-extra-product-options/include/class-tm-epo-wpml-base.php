...
class TM_EPO_WPML_base {
		...
		if ($this->is_wpml){
			if($post_type == 'product'){
				...
				if ( !empty($_GET['tmparentpostid']) && !empty($_GET['tmaddlang']) 
	            	...
	            	$tm_meta_parent_post_id =  get_post_meta($id, 'tm_meta_parent_post_id',true);
		...
		if ($this->is_wpml){
	        ...
	        if($post_type == 'product'){
				...
				if ( !empty($_GET['tmparentpostid']) && !empty($_GET['tmaddlang']) 
	            	...
	            	$tm_meta_parent_post_id =  get_post_meta($product_id, 'tm_meta_parent_post_id',true);
	...
	public function get_terms($lang=null, $taxonomy="", $args=array()){
		...
		if ($this->is_wpml){
			...
			if (
                ...
                if (!empty($post_id)){
                	...
                	$tm_meta_parent_post_id  = get_post_meta( $post_id ,'tm_meta_parent_post_id', true);
	...
	public function add_meta_box(){
		if ($this->is_wpml){
            add_meta_box( 'icl_div', __( 'Language', TM_EPO_TRANSLATION ), array( $this, 'meta_box' ), null, 'side', 'high' );
        }
	}
	...
	public function meta_box($post){
		if ($this->is_wpml){
			...
			$tm_meta_parent_post_id = get_post_meta( $post->ID , 'tm_meta_parent_post_id' , true );
			$is_original = false;
			...
			TM_EPO_HTML()->tm_make_field( array(
					...
					"type"		=> "hidden",
					"tags"		=> array( "id"=>"tm_meta_parent_post_id", "name"=>"tm_meta_parent_post_id" )
				), 1 );
			TM_EPO_HTML()->tm_make_field( array(
					"nodiv" 	=> 1,
					"id" 		=> "tmaddlang",
