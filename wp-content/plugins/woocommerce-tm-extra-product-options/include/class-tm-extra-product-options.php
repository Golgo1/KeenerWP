...
final class TM_Extra_Product_Options {
			...
			if ($has_epo){
				...
				foreach ($epos as $key => $epo) {
					if ($epo && is_array($epo) && isset($epo['section'])){
						...
						$items[$item_id]['item_meta'][$epo['name']][] = $epo['value'];
					}
				}
			}
		}
	...
	public function is_quick_view(){
		...
		if ( $woothemes_quick_view || $theme_flatsome_quick_view || $theme_kleo_quick_view ){
			$qv=true;
		}
	...
	public function get_price_for_cart($price=0,$cart_item=array(),$symbol=false){
		global $woocommerce;
		$product 			= $cart_item['data'];
		...
		if ($per_product_pricing){
			...
			switch ($_price_type) {
				...
				case 'charpercent':
					...
					break;
				case 'charnofirst':
					$_textlength=floatval(strlen(stripcslashes($_POST[$attribute])))-1;
					if ($_textlength<0){
						$_textlength=0;
					}
				...
				if(isset($current_builder[$element])){
					if($index!==false){
						if(isset($current_builder[$element][$index])){
							return $this->tm_apply_filter($current_builder[$element][$index],$apply_filters);
						}else{
							return $this->tm_apply_filter($alt,$apply_filters);
						}
					}else{
						return $this->tm_apply_filter($current_builder[$element],$apply_filters);
					}
				}
			}
			if($index!==false){
								...
								if ( isset( $_elements[$k0] ) && isset($this->tm_original_builder_elements[$_elements[$k0]]) ) {
									...
									if ($this->tm_original_builder_elements[$_elements[$k0]]){
										if ($this->tm_original_builder_elements[$_elements[$k0]]["type"]=="single"){
											...
											if ( isset( $builder['multiple_'.$_elements[$k0].'_options_price'][$_counter[$_elements[$k0]]] ) ) {
												...
												$_images=$this->get_builder_element('multiple_'.$_elements[$k0].'_options_image',$builder,$current_builder,$_counter[$_elements[$k0]],array(),true,$_elements[$k0],'tm_image_url');
												$_imagesp=$this->get_builder_element('multiple_'.$_elements[$k0].'_options_imagep',$builder,$current_builder,$_counter[$_elements[$k0]],array(),true,$_elements[$k0],'tm_image_url');
									...
									if ($_elements[$k0]!="header" && $_elements[$k0]!="divider"){										
										...
										array(
											...
											'clear_options' 	=> $this->get_builder_element($_prefix.'clear_options',$builder,$current_builder,$_counter[$_elements[$k0]],"",$wpml_element_fields,$_elements[$k0]),
